...
// Memoized per station: React.memo's shallow compare keeps unchanged stations
// (and their Leaflet markers) untouched across unrelated map re-renders.
const StationMarkerBase: React.FC<StationMarkerProps> = ({
  station,
  activeModel,
  activeVariable,
  hourIndex,
  ...
  const weatherCode = station.weatherCodes[hourIndex] ?? 0;
  const condition = getWeatherDescription(weatherCode);
  const colors = getBadgeColor(activeVariable, val);
  const customIcon = createStationIcon(station.name, valStr, condition.icon, colors, isSelected);
      ...
      <Popup>
        {popupOpen && (
          <div className="p-1 min-w-[150px]">
            ...
            <div className="text-xs text-slate-400">{station.admin}</div>
...
const StationMarker = React.memo(StationMarkerBase);
    ...
    <>
      {stations.map((st) => (
        <StationMarker
          ...
          hourIndex={hourIndex}
          isSelected={
            Math.abs(currentLocation.latitude - st.lat) < 0.05 &&
            Math.abs(currentLocation.longitude - st.lon) < 0.05
          }
