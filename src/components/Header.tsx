...
const HeaderBase: React.FC<HeaderProps> = ({ onOpenInfo }) => {
  return (
    <header className="border-b border-slate-800/60 bg-slate-900/95 sticky top-0 z-30 shadow-lg shadow-slate-950/50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-2 sm:py-4">
        <div className="flex items-center justify-between gap-2 sm:gap-4">
          {/* Left: Logo and App Info */}
          <div className="flex items-center gap-2 sm:gap-4 min-w-0">
            <div className="relative">
              <div className="absolute inset-0 bg-gradient-to-r from-blue-500 via-indigo-500 to-cyan-400 rounded-xl blur opacity-40" />
            ...
            <div className="min-w-0">
              <div className="flex items-center gap-2 flex-wrap">
                ...
                <span className="hidden sm:inline px-2.5 py-1 rounded-full text-[11px] font-semibold bg-gradient-to-r from-emerald-500/20 to-cyan-500/20 text-emerald-300 border border-emerald-500/30">
                  Modelos IA reales
                </span>
              </div>
          ...
          <div className="flex shrink-0 items-center gap-1 sm:gap-2.5 justify-end">
            <button
              onClick={onOpenInfo}
              className="flex items-center gap-2 px-3 sm:px-4 py-1.5 rounded-lg text-xs font-medium bg-slate-800/60 hover:bg-slate-700/80 text-slate-200 border border-slate-700/60 transition-all hover:shadow-lg hover:shadow-blue-500/10"
