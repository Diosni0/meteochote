...
// In-memory cache for weather data (TTL: 20 minutes)
const weatherCache = new Map();
const CACHE_TTL = 20 * 60 * 1000;

// Dynamic Google Weather API Key storage (can be set via UI or .env)
let configuredGoogleApiKey = process.env.GOOGLE_MAPS_API_KEY || '';

// Reference Spanish cities for the interactive map
...
app.get('/api/health', (req, res) => {
  res.json({
    ...
    googleApiConfigured: Boolean(configuredGoogleApiKey),
  });
});

// Save or test Google Maps Weather API Key
app.post('/api/config/google-key', async (req, res) => {
  ...
  try {
    const testUrl = `https://weather.googleapis.com/v1/currentConditions:lookup?key=${encodeURIComponent(apiKey.trim())}&location.latitude=40.4168&location.longitude=-3.7038`;
    const response = await axios.get(testUrl, { timeout: 5000 });

    if (response.status === 200) {
      configuredGoogleApiKey = apiKey.trim();
      return res.json({ success: true, message: 'Clave API de Google Maps validada y activada para WeatherNext 3.' });
    }
  } catch (error) {
    ...
    return res.status(400).json({
      success: false,
      error: `Error al validar clave de Google: ${msg} (HTTP ${status}). Puedes continuar en modo de simulación de alta fidelidad.`,
    });
  }
});
...
app.get('/api/search', async (req, res) => {
  ...
  try {
    const url = `https://geocoding-api.open-meteo.com/v1/search?name=${encodeURIComponent(query)}&count=12&language=es&format=json`;
    ...
    res.status(500).json({ error: 'Error al buscar localidades', results: [] });
  }
});

// Spain overview endpoint for the interactive map (batched request for 20 stations)
...
// Single location detailed forecast endpoint
app.get('/api/forecast', async (req, res) => {
  ...
  try {
    ...
    if (configuredGoogleApiKey) {
      try {
        const testRes = await axios.get(`https://weather.googleapis.com/v1/forecast/days:lookup?key=${configuredGoogleApiKey}&location.latitude=${lat}&location.longitude=${lon}`, { timeout: 4000 });
        if (testRes.status === 200) isWeatherNextLive = true;
      } catch (err) {
        console.warn('Google Weather API warning:', err.message);
      }
    }

    const hoursCount = omData.hourly?.time?.length || 168;
    ...
    const sevenDayForecast = dailyDates.slice(0, 7).map((dateStr, idx) => {
      ...
      const precipProb = dailyRaw.precipitation_probability_max?.[idx] ?? Math.max(...aiModels.ai_consensus.precip.slice(startH, endH), 0);
...
app.listen(PORT, () => {
  console.log(`Backend de MeteoIA España ejecutándose en http://localhost:${PORT}`);
});
